...
const ECONOMY = Object.freeze(window.VCS_ECONOMY_DATA);
const OPENING = Object.freeze(ECONOMY.openingState);
const REV_PER_USER_DAY = ECONOMY.revenuePerUserDay;
const DAY8_MULTIPLIER = ECONOMY.day8InferenceMultiplier;
const CREDIT_PACK = ECONOMY.creditPack.credits;
const DAY7_PACK_PRICE = ECONOMY.creditPack.day7Price;
const DAY8_PACK_PRICE = ECONOMY.creditPack.day8Price;
const OPERATING_HOURS = ECONOMY.operatingDayHours;
const ENERGY_MAXIMUM = ECONOMY.energyMaximum;
const ORACLE_DAY7_TEST_STRATEGY = Object.freeze(ECONOMY.oracleTestingStrategies.day7SecondPhoto);
...
const scaledEconomicAmount = (spec,{inflation=1,marketStage=1}={}) => Math.round(Number(spec.baseAmount||0)*Math.pow(inflation,Number(spec.inflationElasticity||0))*Math.pow(marketStage,Number(spec.marketStageElasticity||0))*100)/100;
...
/* The public ClearRead route compresses the same four observable requirements
   ...
   presentation; the surrounding HQ, ledger and settlement remain V1. */
...
const configuredAction = action => Object.freeze({
  cash:Number(action?.cash||0),credits:Number(action?.credits||0),
  hours:Number(action?.durationMinutes||0)/60,energy:Number(action?.energy||0)
});
const configuredActionGroup = group => Object.freeze(Object.fromEntries(
  Object.entries(group||{}).map(([name,action])=>[name,configuredAction(action)])
));
const ORACLE_V2_COSTS = configuredActionGroup(ECONOMY.actionCosts.oracleDay7);
const ORACLE_DAY8_COSTS = configuredActionGroup(ECONOMY.actionCosts.oracleDay8);
const TEST_BENCH_COST = configuredAction(ECONOMY.actionCosts.testBench);
...
const DAY7_TASKS = {
  glare:{
    ...
    person:'USER_0047\u2019s grandma', type:`Read-aloud app \u00b7 ${OPENING.activeUsers} users`,
    title:'Stop wrong reads on shiny labels',
    problem:'Last night ClearRead read a glossy medicine label wrong, out loud, with total confidence. She nearly followed the wrong dose.',
    takes:'Two decisions become four saved behaviors \u00b7 about 3\u20134 hours to build, test, and revise.',
    ignoreUsers:ECONOMY.storyEconomy.day7SkippedUsers,
    ignoreLine:`${ECONOMY.storyEconomy.day7SkippedUsers} users will cancel tomorrow.`,
    caseName:'the shiny-label case',
    payoffLine:'HQ Demo Night pays for a working demo before judging closes at 17:00.',
    eventName:'HQ Demo Night', deadline:17, sponsorSpec:ECONOMY.storyEconomy.hqDemoReward, hypeOdds:0.5,
...
/* ============================================================
   ...
   ============================================================ */
const initialState = () => ({
  day:7, time:9, cash:OPENING.cash, credits:OPENING.prepaidCredits, energy:OPENING.energy, users:OPENING.activeUsers,
  aiDaily:OPENING.aiDaily,
  inflationMultiplier:1, marketStageMultiplier:1,
  // pipeline
  ...
  oracleV2:{stage:'dialogue',question:0,answers:{},behaviors:[],secondPhoto:null,testStrategy:'',diagnosis:'',patch:'',evidence:[],release:null},
  ...
  activityResults:{}, activityEventHistory:{}, activityLedger:[], postedActivityEntries:{}, learnedLessons:[], unlockedTests:[],
  economyLedger:[],
  storyFlags:[],
  // consequences ledger
  exposures:[], trustPenalty:0, fitAtShip:'closed', _blewOvernight:false,
  // day accounting
  dayStartCash:OPENING.cash, dayStartAiDaily:OPENING.aiDaily, opRevenue:0, opAI:0, sponsorCash:0, preorderCash:0,
...
const dailyRental = () => scaledEconomicAmount(ECONOMY.dailyRental,{inflation:activityInflation(),marketStage:S.marketStageMultiplier||1});
const projectedDayChange = () => dailyProductResult() - dailyRental();
const runway = () => S.cash / Math.max(1, dailyRental() - dailyProductResult());
...
const taskSponsor = t => scaledEconomicAmount(t?.sponsorSpec||{
  baseAmount:Number(t?.sponsor||0),inflationElasticity:1,marketStageElasticity:1
},{inflation:activityInflation(),marketStage:S.marketStageMultiplier||1});
const taskPayoffLine = t => {
  if(!t)return'';
  if(t.id==='glare')return `${eventName(t)} pays ${plainMoney(taskSponsor(t))} for a working demo before judging closes at ${clockText(eventDeadline(t))}.`;
  return String(t.payoffLine||'').replace(/\$\d+(?:\.\d{1,2})?/,plainMoney(taskSponsor(t)));
};
...
function chooseActivityEvent(activity){
  const approved=activity.eventTable.filter(row=>activityAssetsForEvent(activity,row).length);
  const seen=new Set(S.activityEventHistory?.[activity.id]||[]);
  const available=approved.filter(row=>!seen.has(row.id)).length?approved.filter(row=>!seen.has(row.id)):approved;
  ...
  const selected=available.find(row=>((cursor-=row.weight)<=0))||available[available.length-1];
  S.activityEventHistory[activity.id]=[...(S.activityEventHistory[activity.id]||[]),selected.id];
  return selected;
}
...
function applyActivityEffect(activity,effect,event,index){
  ...
  if(['energy','cash','users'].includes(effect.type))postEconomyEntry({
    sourceId:entry.id,label:entry.label,resource:effect.type,amount,
    metadata:{activityId:activity.id,eventId,activityEntryId:entry.id}
  });
...
function postActivitySettlementEntries(){
  ...
  activityLedgerEntries().forEach(entry=>{
    ...
    if(['cash','users'].includes(entry.resource))postEconomyEntryOnce({
      sourceId:entry.id,label:entry.label,resource:entry.resource,amount:entry.amount,
      metadata:{activityId:entry.source.id,eventId:entry.eventId,activityEntryId:entry.id}
    });
...
function activityImpact(activity){
  ...
  entries.filter(entry=>entry.resource==='lesson'||entry.resource==='test-unlock').forEach(entry=>{
    const note=entry.resource==='test-unlock'
      ? `Future test saved: ${entry.label||entry.detail}.`
...
function resetModalScroll(){
  ...
  modal.querySelectorAll('.oracle-v2-copy,.market-v2-copy,.modal-body').forEach(panel=>{panel.scrollTop=0;});
}

function openModal(content, closable=true){
  $('overlay').hidden=false;$('modal').innerHTML=content;resetModalScroll();
  requestAnimationFrame(resetModalScroll);
  if(closable)$('modal').querySelectorAll('[data-close]').forEach(close=>close.onclick=closeModal);
...
function resourceBar(){return `<div class="resource-bar" aria-label="Resources before this action"><span>Cash <b>${plainMoney(S.cash)}</b></span><span>AI credits <b>${Math.floor(S.credits)} ⚡</b></span><span>Energy <b>${S.energy}/${ENERGY_MAXIMUM}</b></span><span>Clock <b>${timeText()}</b></span></div>`;}
...
function render(){
  ...
  $('hud-cash-note').textContent = `Daily rental: ${plainMoney(dailyRental())}`;
  $('hud-cash-note').style.color = 'var(--red)';
  ...
  $('hud-energy').textContent = `${S.energy} / ${ENERGY_MAXIMUM}`;
  ...
  $('eco-ai').textContent = S.pendingLaunch&&!S.settled
    ...
    : fmtDay(-S.aiDaily);
  $('eco-fixed').textContent = `−${plainMoney(dailyRental())}`;
  const dayChange = projectedDayChange();
  $('eco-day').textContent = money(dayChange);
  $('eco-day').className = dayChange>=0?'good':'bad';
  $('impact').innerHTML = t
    ? `<b>${t.person} · ${t.title}</b>${t.problem}<div style="margin-top:6px"><strong>If skipped:</strong> ${t.ignoreLine}</div><div><strong>${eventName(t)} · ${clockText(eventDeadline(t))}:</strong> ${taskPayoffLine(t)}</div>`
  ...
  $('game-title').textContent = `HQ · Day ${S.day}`;
...
function stationAction(key){
  ...
  else if(key==='test')(['oracle-v2','oracle-v2-day8'].includes(S.build?.kind)||(!S.build&&((day7()&&task()?.id==='glare')||(!day7()&&task()?.id==='offline'))))?showOracleV2TestBench():showTest();
  else if(key==='activity')showActivities();
  else if(key==='coffee')showRecoveryMenu();
  else if(key==='door')showDoor();
}
  ...
  showScene({
    ...
    pages:[
      ...
      {speaker:'FOUNDER · 09:00, DAY 7', title:'Morning', html:`
        <p>84 people use ClearRead every day. Last night it failed one of the people it was built for.</p>
        <p>What will you do to make it right?</p>`}
    ],
...
function showTaskBoard(){
  ...
  const focusedTask=focused?set[ids[0]]:null;
  ...
  <div class="modal-body"><div class="task-grid ${focused?'single-task':''}">${ids.map((id,i)=>{const t=set[id];return `
    <article class="task-card ${i===0?'featured':''}">
      ...
      <div class="inner">
        ...
        <div class="receipt gain"><b>${eventName(t)} · closes ${clockText(eventDeadline(t))}</b><div class="r-row"><span>${taskPayoffLine(t)}</span></div></div>
        <button class="btn ${i===0?'primary':'cyan'}" data-task="${id}">${taskButtonLabel(t)}</button>
      </div>
    </article>`;}).join('')}</div>
    <div class="modal-actions"><button class="btn" id="defer-product-request">Not today — return to HQ</button></div><div class="plain-rule">You can spend the day elsewhere. If you end the day without resolving this request, ${focusedTask?.ignoreLine||'it remains unresolved.'}</div>
  </div>`, false);
...
function showInviteScene(){
  ...
  showScene({
    pages:[{speaker:'10:02 · TODAY’S OPPORTUNITY', title:eventName(t), html:`
      ...
      <p>${taskPayoffLine(t)}</p></div>
      <div class="phone-msg"><span class="from">HQ</span><span class="when">MIDNIGHT</span>
      <p>The event closes at <b>${deadline}</b>. Releasing from HQ stays available until midnight.</p></div>`,
      aside:`The deadline is tied to this customer’s schedule and the task’s value. Missing it loses the ${plainMoney(taskSponsor(t))} payment opportunity; it does not end the day.`}],
    doneLabel:'Got it',
...
function showOracleV2Question(){
  ...
  oracleV2Shell({art:index===0?ASSET.user47:ASSET.oracle,eyebrow:`ORACLE STUDIO · DECISION ${index+1} OF 2`,title:q.title,
    lede:index===0?`ClearRead 0.7.4 is already live for ${OPENING.activeUsers} people. You are repairing the first app you built earlier this week.`:'This decision records the recovery behavior and the evidence the build must preserve.',
    body:customer,
    ...
    footer:'<button class="btn" id="v2-back-hq">Back to HQ</button>'});
  $('v2-back-hq').onclick=closeModal;
...
function showOracleV2Brief(){
  ...
  oracleV2Shell({art:ASSET.oracleBuild,artClass:'build',eyebrow:'ORACLE · CLEARREAD 0.7.5 BUILD PLAN',title:'Your two decisions become four saved behaviors.',
    lede:'Review the exact product behavior before spending Cash, Credits, time, or Energy.',body:oracleV2BehaviorList()+`<div class="plain-rule"><b>Build cost:</b> ${packText} ${formatDuration(cost.hours*60)} · ${cost.energy} Energy.</div>`,
    ...
    footer:'<button class="btn" id="v2-edit">Change the two decisions</button><button class="btn" id="v2-cancel">Back to HQ</button>'});
...
function saveOracleV2Build(needsPack){
  const cost=ORACLE_V2_COSTS.build;if(S.build?.kind==='oracle-v2'||S.energy<cost.energy)return;
  if(needsPack&&!purchaseCreditPack('oracle-v2:day7-credit-pack',DAY7_PACK_PRICE))return;
  if(S.credits<cost.credits)return;
  if(!postActionCosts('oracle-v2:day7-build','Build ClearRead 0.7.5',cost))return;S.productVersion='0.7.5';S.version=5;
...
function showOracleV2DevTestOffer(){
  ...
  oracleV2Shell({art:ASSET.dev,eyebrow:'DEV · BUILD 0.7.5 REVIEW',title:'“Let’s prove the repair against the photo that exposed it.”',
    ...
    choices:`<button class="oracle-v2-choice primary" id="v2-test-now"><strong>Test the reported photograph now</strong><span>Open the customer evidence · ${formatDuration(cost.hours*60)} · ${cost.credits} Credits</span></button><button class="oracle-v2-choice" id="v2-test-later"><strong>Save it for the Test Bench</strong><span>Return to HQ. Dev will keep the same test ready there.</span></button>`,
...
function showOracleV2CustomerTest(){
  ...
  oracleV2Shell({art:ASSET.user47,eyebrow:'CUSTOMER EVIDENCE · REPORTED PHOTO',title:'Does 0.7.5 fix the failure USER_0047 reported?',
    ...
    footer:'<button class="btn" id="v2-pause">Return to HQ</button>'});
  $('v2-pause').onclick=closeModal;
  $('v2-customer-test').onclick=()=>{
    if(!postActionCosts('oracle-v2:reported-photo-test','Test reported photograph',cost))return;S.tested=true;
...
function showOracleV2DevChoice(){
  const cost=ORACLE_V2_COSTS.test,strategies=ORACLE_DAY7_TEST_STRATEGY.choices;
  oracleV2Shell({art:ASSET.dev,eyebrow:'CUSTOMER PHOTO · PASSED',title:'The reported failure is fixed.',
    ...
    choices:`<button class="oracle-v2-choice primary" id="v2-second"><strong>${strategies.test.label}</strong><span>${strategies.test.description}</span></button><button class="oracle-v2-choice" id="v2-release-one"><strong>${strategies.skip.label}</strong><span>${strategies.skip.description}</span></button>`});
  $('v2-second').onclick=()=>{S.oracleV2.testStrategy='test';runOracleV2SecondPhoto();};
  $('v2-release-one').onclick=()=>{S.oracleV2.testStrategy='skip';S.oracleV2.secondPhoto='untested';S.oracleV2.stage='release';showOracleV2Release();};
}

function runOracleV2SecondPhoto(){
  const cost=ORACLE_V2_COSTS.test;
  if(!postActionCosts('oracle-v2:second-photo-test','Test second clear photograph',cost))return;const pass=oracleV2SecondPhotoPasses();S.oracleV2.secondPhoto=pass?'pass':'fail';
...
function showOracleV2Patch(){
  ...
  $('v2-patch').onclick=()=>{
    if(!postActionCosts('oracle-v2:revision','Revise and rerun both photographs',cost))return;
...
function showOracleV2Release(){
  ...
  const release=ORACLE_V2_COSTS.release;
  const strategy=S.oracleV2.testStrategy||'test';
  const allowedScopes=ORACLE_DAY7_TEST_STRATEGY.choices[strategy].releaseScopes;
  const wideAllowed=allowedScopes.includes('wide');
  oracleV2Shell({art:ASSET.oracleBuild,artClass:'build',eyebrow:`CLEARREAD ${S.productVersion} · RELEASE`,title:'How far should this build travel?',
    lede:`Proven today: ${evidence}. Choose where the tested build becomes available. Results appear after the day ends.`,
    body:`<div class="scope-grid"><div class="scope-box"><h3>Proven</h3><ul>${S.oracleV2.evidence.filter(e=>e.result==='pass').map(e=>`<li>${e.label}</li>`).join('')}</ul></div><div class="scope-box risk"><h3>Not proven</h3><ul>${second==='untested'?'<li>Second-photo sessions</li>':''}<li>Other languages</li><li>New label formats</li><li>Untested lighting</li></ul></div></div><div class="plain-rule"><b>${strategy==='skip'?'You saved 30 minutes and 20 Credits.':'You bought evidence about repeat scans.'}</b> Release preparation takes ${formatDuration(release.hours*60)}. New users and revenue are calculated after the day ends.</div>`,
    choices:`<button class="oracle-v2-choice" data-v2-scope="evidence"><strong>Release to the tested cases</strong><span>Keep the launch inside today’s evidence.</span></button><button class="oracle-v2-choice primary" data-v2-scope="wide" ${wideAllowed?'':'disabled'}><strong>Release to the wider market</strong><span>${wideAllowed?'Invite use in cases this build has not demonstrated.':'Test a second photo first to support a wider release.'}</span></button>`,
...
function recordOracleV2Release(scope){
  const strategy=S.oracleV2.testStrategy||'test';
  if(!ORACLE_DAY7_TEST_STRATEGY.choices[strategy].releaseScopes.includes(scope)){toast('A wider release needs the second-photo test.');return;}
  if(S.shipped||!postActionCosts('oracle-v2:day7-release','Record ClearRead release',ORACLE_V2_COSTS.release))return;
  S.shipped=true;S.copyType=scope;S.copy=scope==='evidence'?'ClearRead repair released to the tested cases.':'ClearRead repair opened to the wider market.';
  S.oracleV2.release={scope,version:S.productVersion,testStrategy:strategy,status:'market result pending',evidence:S.oracleV2.evidence.map(e=>({...e}))};
...
function showOracleV2TestBench(){
  if(!S.build){
    oracleV2Shell({art:ASSET.dev,eyebrow:`DEV · TEST BENCH · DAY ${S.day}`,title:'“Nothing is queued yet.”',
      ...
      choices:'<button class="oracle-v2-choice primary" id="bench-to-oracle"><strong>Go to ORACLE Studio</strong><span>Define and save the build before testing.</span></button>',footer:'<button class="btn" id="bench-empty-back">Back to HQ</button>'});
  ...
  oracleV2Shell({art:ASSET.dev,eyebrow:`DEV · TEST BENCH · DAY ${S.day}`,title:dayLine,
    ...
    footer:`${day7()&&S.oracleV2.stage==='customer-pass'?'<button class="btn" id="bench-dev-second">Ask about one more check</button>':''}<button class="btn" id="bench-done-back">Back to HQ</button>`});
...
function showOracleDay8QuestionV2(){
  oracleV2Shell({art:ASSET.oracle,eyebrow:'ORACLE STUDIO · DAY 8 · FOCUSED UPDATE',title:'What should ClearRead do when the clinic loses Wi-Fi?',
    ...
    footer:'<button class="btn" id="day8-v2-back">Back to HQ</button>'});
...
function showOracleDay8BriefV2(){
  ...
  oracleV2Shell({art:ASSET.oracleBuild,artClass:'build',eyebrow:'ORACLE · CLEARREAD 0.8.0 BUILD PLAN',title:'Keep yesterday’s build. Add one offline rule.',
    lede:`The ${CREDIT_PACK}-Credit update package covers the implementation and its clinic replay: ${build.credits} Credits to build, then ${test.credits} Credits to test.`,
    body:day8BehaviorList()+`<div class="plain-rule"><b>Day 8 receipt:</b> Buy ${CREDIT_PACK} Credits for ${plainMoney(DAY8_PACK_PRICE)}. Build: ${build.credits} Credits · ${formatDuration(build.hours*60)} · ${build.energy} Energy. Test: ${test.credits} Credits · ${formatDuration(test.hours*60)}. Cash and Credits post only after confirmation.</div>`,
    choices:`<button class="oracle-v2-choice primary" id="day8-v2-buy"><strong>Buy ${CREDIT_PACK} Credits and build 0.8.0 · ${plainMoney(DAY8_PACK_PRICE)}</strong><span>Cash ${plainMoney(S.cash)} → ${plainMoney(S.cash-DAY8_PACK_PRICE)} · Credits ${Math.floor(S.credits)} → ${Math.floor(S.credits+CREDIT_PACK-build.credits)} after the build</span></button>`,
    footer:'<button class="btn" id="day8-v2-edit">Change the offline rule</button><button class="btn" id="day8-v2-cancel">Back to HQ</button>'});
...
function showOracleDay8PurchaseConfirm(){
  ...
  oracleV2Shell({art:ASSET.oracleBuild,artClass:'build',eyebrow:'CONFIRM · DAY 8 CREDIT PURCHASE',title:`Spend ${plainMoney(DAY8_PACK_PRICE)} on the update package?`,
    ...
    choices:`<button class="oracle-v2-choice primary" id="day8-v2-confirm" ${canAfford?'':'disabled'}><strong>Buy Credits and build the update</strong><span>${formatDuration(cost.hours*60)} · ${cost.energy} Energy · clinic test remains ready at the Test Bench</span></button>`,
...
function saveOracleDay8Build(){
  ...
  if(!beginTimedAction('Build the ClearRead update',cost.hours*60))return;
  if(!purchaseCreditPack('oracle-v2:day8-credit-pack',DAY8_PACK_PRICE))return;
  if(!postActionCosts('oracle-v2:day8-build','Build ClearRead 0.8.0',cost))return;S.productVersion='0.8.0';S.version=8;
...
function showOracleDay8DevTestOffer(){
  ...
  oracleV2Shell({art:ASSET.dev,eyebrow:'DEV · DAY 8 BUILD REVIEW',title:'“Westside sent us an outage replay.”',
    ...
    choices:`<button class="oracle-v2-choice primary" id="day8-test-now"><strong>Run the clinic outage replay now</strong><span>${formatDuration(cost.hours*60)} · ${cost.credits} Credits · completes the ${CREDIT_PACK}-Credit update package</span></button><button class="oracle-v2-choice" id="day8-test-later"><strong>Save it for the Test Bench</strong><span>Return to HQ. Dev will keep the outage replay queued.</span></button>`,
...
function showOracleDay8Test(){
  ...
  oracleV2Shell({art:ASSET.dev,eyebrow:'TEST BENCH · WESTSIDE CLINIC',title:'Does 0.8.0 behave correctly when the connection disappears?',
    ...
    choices:`<button class="oracle-v2-choice primary" id="day8-run-test"><strong>Run the outage replay</strong><span>${cost.hours*60} minutes · ${cost.credits} Credits · no Energy</span></button>`,footer:'<button class="btn" id="day8-test-back">Back to HQ</button>'});
  $('day8-test-back').onclick=closeModal;$('day8-run-test').onclick=runOracleDay8Test;
}

function runOracleDay8Test(){
  const cost=ORACLE_DAY8_COSTS.test;if(S.tested||S.credits<cost.credits)return;
  if(!postActionCosts('oracle-v2:day8-outage-test','Run Westside Clinic outage replay',cost))return;S.tested=true;S.issueResolved=true;S.testResults=[{name:'Westside Clinic outage replay',detail:S.oracleDay8.behavior,slot:'offline',pass:true}];
...
function showOracleDay8Result(){
  oracleV2Shell({art:ASSET.oracle,eyebrow:'CLEARREAD 0.8.0 · CLINIC REPLAY PASSED',title:'The saved offline rule works.',
    ...
    choices:'<button class="oracle-v2-choice primary" id="day8-record-release"><strong>Record the 0.8.0 update</strong><span>Carry yesterday’s release scope forward; market response remains pending until settlement.</span></button>',footer:'<button class="btn" id="day8-result-back">Back to HQ</button>'});
  $('day8-result-back').onclick=closeModal;$('day8-record-release').onclick=recordOracleDay8Release;
}

function recordOracleDay8Release(){
  if(S.shipped||!postActionCosts('oracle-v2:day8-release','Record ClearRead 0.8.0 update',ORACLE_DAY8_COSTS.release))return;
  S.shipped=true;
  ...
  <div class="modal-body">
    ...
    <div class="build-choice">
      <div class="build-card">
        ...
        <button class="btn cyan" id="build-quick" ${quickRepeat||S.credits<quick.credits||S.energy<quick.energy?'disabled':''}>${quickRepeat?'Already saved — test this build':'Buy the quick build'}</button>
      </div>
      <div class="build-card careful">
        ...
        <button class="btn primary" id="build-careful" ${carefulRepeat||S.credits<careful.credits||S.energy<careful.energy?'disabled':''}>${carefulRepeat?'Already saved — test this build':'Buy the careful build'}</button>
      </div>
    </div>
    ...
    <div class="modal-actions">${S.credits<quick.credits?`<button class="btn" id="day7-buy-credits" ${S.cash<DAY7_PACK_PRICE?'disabled':''}>Buy ${CREDIT_PACK} ⚡ · ${plainMoney(DAY7_PACK_PRICE)}</button>`:''}<button class="btn" data-close>Back to HQ</button></div>
  </div>`);
...
function generateDay7(kind,cost){
  if(repeatBuildBlocked(kind)||S.credits<cost.credits||S.energy<cost.energy)return;
  const before={credits:S.credits,energy:S.energy,cash:S.cash,time:timeText()};
  if(!postActionCosts(`legacy-oracle:day7-${kind}`,`Generate ${kind} Day 7 build`,cost))return;
  S.version+=1;
  ...
  <div class="modal-body">
    ...
    <div class="build-choice">
      <div class="build-card"><h3>Standard build</h3>
        ...
        <button class="btn cyan" id="build-standard" ${standardRepeat||coverageCount()<1||S.credits<standard.credits||S.energy<standard.energy?'disabled':''}>${standardRepeat?'Already saved — test this build':'Buy the Standard build'}</button>
      </div>
      <div class="build-card careful"><h3>Reviewed build</h3>
        ...
        <button class="btn primary" id="build-reviewed" ${reviewedRepeat||coverageCount()<1||S.credits<reviewed.credits||S.energy<reviewed.energy?'disabled':''}>${reviewedRepeat?'Already saved — test this build':'Buy the Reviewed build'}</button>
      </div>
    </div>
...
function confirmCreditPurchase(returnTo=showDay8BuildChoice){
  ...
  $('credit-confirm').onclick=()=>{if(!purchaseCreditPack(`manual-credit-pack:day-${S.day}`,price))return;toast(`AI Credits purchased · cash ${plainMoney(beforeCash)} → ${plainMoney(S.cash)}`);returnTo();render();};
}

function generateDay8(kind,cost){
  if(cost.credits>S.credits||S.energy<cost.energy)return;
  const before={credits:S.credits,energy:S.energy,cash:S.cash};
  if(!postActionCosts(`legacy-oracle:day8-${kind}`,`Generate ${kind} Day 8 build`,cost))return;
  S.version+=1;
...
function showTest(){
  if(!S.build){openModal(`<div class="modal-head"><div><div class="micro">Test bench</div><h2 id="modal-title">Nothing to test yet</h2><p>Describe the build at ORACLE first — the tests replay the incident against whatever you actually asked for.</p></div><button class="close" data-close>×</button></div><div class="modal-body"><div class="modal-actions"><button class="btn primary" id="go-oracle">Open ORACLE Studio</button></div></div>`);$('modal').querySelector('[data-close]').onclick=closeModal;$('go-oracle').onclick=()=>{closeModal();showOracle();};return;}
  if(!S.tested){
    const rows=runTestRows(),cost=TEST_BENCH_COST;
    openModal(`<div class="modal-head"><div><div class="micro">Test bench · ${task().app} · build v${S.version}</div><h2 id="modal-title">Replay the incident</h2><p>${task().person}’s case, re-run against the saved build · ${cost.credits} prepaid ⚡ · ${cost.energy} Energy · ${formatDuration(cost.hours*60)}.</p>${resourceBar()}</div><button class="close" data-close>×</button></div>
    <div class="modal-body">
      ...
      ${S.energy<cost.energy?`<div class="plain-rule" style="border-left-color:var(--yellow)">Not enough Energy to test — take a break to recover.</div>`:''}
      <div class="modal-actions"><button class="btn" data-close>Back</button><button class="btn primary" id="run-tests" ${canAffordAction(cost)?'':'disabled'}>Run the tests · ${cost.credits} ⚡ · ${cost.energy} Energy · ${formatDuration(cost.hours*60)}</button></div>
    </div>`);
    $('modal').querySelectorAll('[data-close]').forEach(b=>b.onclick=closeModal);
    const run=$('run-tests');if(run&&!run.disabled)run.onclick=()=>{const before={credits:S.credits,energy:S.energy,cash:S.cash};if(!postActionCosts(`test-bench:day-${S.day}`,'Replay the incident',cost))return;S.tested=true;S.testResults=runTestRows();toast(`Test run: ${before.credits}→${S.credits} ⚡ · Energy ${before.energy}→${S.energy} · cash ${plainMoney(before.cash)}→${plainMoney(S.cash)}`);showTestResults();render();};
    return;
  }
  showTestResults();
}
...
function showTestResults(){
  ...
  <div class="modal-body">
    ...
    <div class="modal-actions"><button class="btn" data-close>Back to HQ</button>${all?'<button class="btn primary" id="go-door">To customer event / release</button>':''}</div>
  </div>`);
  $('modal').querySelectorAll('[data-close]').forEach(b=>b.onclick=closeModal);
  ...
  openModal(`<div class="modal-head"><div><div class="micro">Garage exit · outside map</div><h2 id="modal-title">Where do you go next?</h2><p>Take as many different activities as the day, your cash, and the schedule allow. Each location is available once per day.</p></div><button class="close" data-close>×</button></div>
    <div class="modal-body"><div class="task-grid">${Object.values(ACTIVITIES).map(a=>{
      ...
      return `<article class="task-card"><div class="inner"><div class="app-type">${a.location} · ${hours}h</div><div class="activity-title"><h3>${a.name}</h3><div class="activity-costs">${activityCostBadges(a)}</div></div><p>${a.preview.description}</p>
        ...
        <button class="btn cyan" data-activity="${a.id}" ${used||!allowed||S.cash<cashCost||S.energy<a.cost.energy?'disabled':''}>${used?'Done today':`Go · ${hours}h${cashCost?` · −${plainMoney(cashCost)}`:''}${a.cost.energy?` · −${a.cost.energy} Energy`:''}`}</button></div></article>`;
    }).join('')}</div>${done.length?`<div class="plain-rule"><b>Today so far:</b> ${done.map(a=>a.name).join(' · ')}. You can choose another distinct activity or return to HQ.</div>`:''}</div>`);
  $('modal').querySelectorAll('[data-close]').forEach(b=>b.onclick=closeModal);
...
function chooseActivity(id){
  ...
  if(didActivity(id)||!activityAvailable(a,t)||S.cash<cashCost||S.energy<a.cost.energy)return;
  if(!beginTimedAction(a.name,a.durationMinutes))return;
  ...
  if(cashCost){
    postEconomyEntry({sourceId:`activity:${a.id}:direct-cost`,label:`${a.name} direct cost`,resource:'cash',amount:-cashCost,durationMinutes:a.durationMinutes,metadata:{activityId:a.id}});
    S.activitySpend+=cashCost;
  }
  recordActivityEntry(a,'direct-cost','cash',-cashCost,'immediate',`${a.name} direct cost`);
  if(a.cost.energy){
    postEconomyEntry({sourceId:`activity:${a.id}:energy-cost`,label:`${a.name} Energy`,resource:'energy',amount:-a.cost.energy,durationMinutes:a.durationMinutes,metadata:{activityId:a.id}});
    recordActivityEntry(a,'energy-cost','energy',-a.cost.energy,'immediate',`${a.name} Energy`);
  }
  ...
  showScene({cls:a.presentation.sceneClass,presentation:{...a.presentation,background},pages,doneLabel:'Return to HQ',onDone:render});
  render();
}
...
/* ============================================================
   ...
   ============================================================ */
function postEconomyEntry({sourceId,label,resource,amount,durationMinutes=0,metadata={}}){
  const sequence=S.economyLedger.filter(entry=>entry.day===S.day&&entry.sourceId===sourceId).length+1;
  const entry=Object.freeze({
    id:`day-${S.day}:economy:${sourceId}:${sequence}`,day:S.day,time:S.time,sourceId,label,resource,
    amount:Number(amount||0),durationMinutes:Number(durationMinutes||0),inflationMultiplier:activityInflation(),
    metadata:Object.freeze({...metadata}),status:'posted'
  });
  S.economyLedger.push(entry);
  if(resource==='cash')S.cash+=entry.amount;
  if(resource==='energy')S.energy=Math.max(0,Math.min(ENERGY_MAXIMUM,S.energy+entry.amount));
  if(resource==='credits')S.credits=Math.max(0,S.credits+entry.amount);
  if(resource==='users')S.users=Math.max(0,S.users+entry.amount);
  if(resource==='aiDaily')S.aiDaily=Math.max(0,S.aiDaily+entry.amount);
  return entry;
}
function postEconomyEntryOnce(args){
  const existing=S.economyLedger.find(entry=>entry.day===S.day&&entry.sourceId===args.sourceId&&entry.resource===args.resource);
  return existing||postEconomyEntry(args);
}
function canAffordAction({cash=0,credits=0,energy=0,hours=0}){
  return S.cash>=cash&&S.credits>=credits&&S.energy>=energy;
}
function minutesLeftToday(){return Math.max(0,Math.round((24-S.time)*60));}
function finishDayFromTimeLimit(){
  if(day7()&&!S.marketCutsceneSeen){showMarketCutscene(true);return;}
  settleDay();
}
function showTimeLimitNotice(label,durationMinutes){
  const remaining=minutesLeftToday();
  openModal(`<div class="modal-head"><div><div class="micro">The day is ending · ${timeText()}</div><h2 id="modal-title">There isn’t enough time left for ${label}.</h2><p>${formatDuration(remaining)} remain before midnight, while this takes ${formatDuration(durationMinutes)}.</p></div></div><div class="modal-body"><div class="plain-rule">Starting now would reach or pass midnight. You stop here and close the day instead.</div><div class="modal-actions"><button class="btn primary" id="time-limit-end-day">Continue to End Day</button></div></div>`,false);
  $('time-limit-end-day').onclick=finishDayFromTimeLimit;
}
function beginTimedAction(label,durationMinutes){
  if(durationMinutes>0&&minutesLeftToday()<=durationMinutes){showTimeLimitNotice(label,durationMinutes);return false;}
  return true;
}
function postActionCosts(sourceId,label,{cash=0,credits=0,energy=0,hours=0},metadata={}){
  const durationMinutes=hours*60;
  if(!canAffordAction({cash,credits,energy,hours})){
    const missing=cash>S.cash?'Cash':credits>S.credits?'AI Credits':'Energy';
    toast(`Not enough ${missing} for ${label}.`);return false;
  }
  if(hours&&!beginTimedAction(label,durationMinutes))return false;
  if(cash)postEconomyEntry({sourceId,label,resource:'cash',amount:-cash,durationMinutes,metadata});
  if(credits)postEconomyEntry({sourceId,label,resource:'credits',amount:-credits,durationMinutes,metadata});
  if(energy)postEconomyEntry({sourceId,label,resource:'energy',amount:-energy,durationMinutes,metadata});
  if(hours)advanceTime(hours);
  return true;
}
function purchaseCreditPack(sourceId,price){
  if(!canAffordAction({cash:price}))return false;
  postEconomyEntry({sourceId,label:'AI Credit pack',resource:'cash',amount:-price,metadata:{credits:CREDIT_PACK}});
  postEconomyEntry({sourceId,label:'AI Credit pack',resource:'credits',amount:CREDIT_PACK,metadata:{cashPrice:price}});
  S.purchaseSpend+=price;
  return true;
}
...
function recoveryCashCost(action){return Math.round(action.baseCashCost*Math.pow(activityInflation(),action.inflationElasticity||0)*100)/100;}
function recoveryAvailable(action){return S.cash>=recoveryCashCost(action);}
function performRecoveryAction(actionId){
  const action=ECONOMY.recoveryActions.find(item=>item.id===actionId);
  if(!action||!recoveryAvailable(action))return;
  if(!beginTimedAction(action.label,action.durationMinutes))return;
  const cashCost=recoveryCashCost(action),beforeCash=S.cash,beforeEnergy=S.energy;
  if(cashCost){
    postEconomyEntry({sourceId:`recovery:${action.id}`,label:action.label,resource:'cash',amount:-cashCost,durationMinutes:action.durationMinutes,metadata:{baseAmount:action.baseCashCost,inflationElasticity:action.inflationElasticity}});
    if(action.id==='coffee')S.coffeeSpend+=cashCost;
  }
  postEconomyEntry({sourceId:`recovery:${action.id}`,label:action.label,resource:'energy',amount:action.energyDelta,durationMinutes:action.durationMinutes});
  advanceTime(action.durationMinutes/60);closeModal();
  toast(`${action.label}: ${cashCost?`${plainMoney(beforeCash)} → ${plainMoney(S.cash)} · `:''}Energy ${beforeEnergy} → ${S.energy}`);render();
}
function showRecoveryMenu(){
  if(S.energy>=ENERGY_MAXIMUM){toast('Energy is already full.');return;}
  const actions=ECONOMY.recoveryActions;
  const sleep=actions.find(action=>action.id==='sleep'),coffee=actions.find(action=>action.id==='coffee');
  openModal(`<div class="modal-head"><div><div class="micro">Take a break</div><h2 id="modal-title">Have a rest, get some coffee!</h2><p>Sleep takes ${formatDuration(sleep.durationMinutes)}. Coffee costs ${plainMoney(recoveryCashCost(coffee))} and takes ${formatDuration(coffee.durationMinutes)}. Both restore ${coffee.energyDelta} Energy.</p></div><button class="close" data-close>×</button></div><div class="modal-body"><div class="break-grid">${actions.map(action=>{const cash=recoveryCashCost(action);return `<button class="oracle-v2-choice ${action.id==='coffee'?'primary':''}" data-recovery="${action.id}" ${recoveryAvailable(action)?'':'disabled'}><strong>${action.label}${cash?` · ${plainMoney(cash)}`:' · free'}</strong><span>${formatDuration(action.durationMinutes)} · +${action.energyDelta} Energy${action.inflationElasticity&&activityInflation()!==1?` · price × ${activityInflation().toFixed(1)}`:''}</span></button>`;}).join('')}</div><div class="modal-actions"><button class="btn" data-close>Cancel</button></div></div>`);
  $('modal').querySelectorAll('[data-recovery]').forEach(button=>button.onclick=()=>performRecoveryAction(button.dataset.recovery));
}
function useCoffee(){showRecoveryMenu();
}
...
function showDoor(){
  ...
  if(!S.demoAttended&&open){
    openModal(`<div class="modal-head"><div><div class="micro">${name} · closes ${clockText(deadline)}</div><h2 id="modal-title">Show the working build</h2><p>One hour. If the tested build works for this customer, ${plainMoney(taskSponsor(t))} is recorded for tonight’s settlement. Public release remains a separate decision.</p></div><button class="close" data-close>×</button></div>
    <div class="modal-body"><div class="modal-actions"><button class="btn" id="skip-demo">Skip the event — release instead</button><button class="btn primary" id="do-demo">Show the build · 1h</button></div></div>`);
...
function presentDemo(verified){
  if(S.demoAttended)return;
  if(!beginTimedAction(eventName(task()),60))return;
  advanceTime(1);S.demoAttended=true;closeModal();
  if(verified){
    const payment=taskSponsor(task());
    ...
    showScene({cls:'stage',pages:[{speaker:eventName(task()).toUpperCase()+' · '+timeText(),title:'The customer accepts the result',html:`
      <p>${task().person}’s case works in front of them. The <b>${plainMoney(payment)}</b> payment is recorded and will post on the midnight receipt.</p>
      <p>An accepted demo is not a public release. Nothing has been promised to the market yet.</p>`}],
      doneLabel:S.shipped?'Return to HQ':'Decide the release',onDone:S.shipped?()=>render():showRelease});
  }else{
...
function shipChoice(type){
  const t=task();if(!beginTimedAction('Record the release',30))return;
...
function showMarketCutscene(forceClose=false){
  ...
  openModal(`<section class="market-v2" style="--summit:url('${ASSET.summit}')">
    ...
    <article class="market-v2-copy"><div class="micro">LATE NIGHT · COMPUTE & CONSEQUENCE</div><h2 id="modal-title">Tomorrow may not cost what today did.</h2>
      ...
      <div class="oracle-v2-choices"><button class="oracle-v2-choice primary" id="market-close-day"><strong>Close Day 7</strong><span>Settle today’s books before the new prices take effect.</span></button>${forceClose?'':'<button class="oracle-v2-choice" id="market-return"><strong>Return to HQ</strong><span>Keep the day open a little longer.</span></button>'}</div>
    </article></section>`,false);
  $('market-close-day').onclick=settleDay;
  const returnButton=$('market-return');if(returnButton)returnButton.onclick=closeModal;
}
...
function confirmEndDay(){
  ...
  openModal(`<div class="modal-head"><div><div class="micro">End the day · ${timeText()}</div><h2 id="modal-title">Done for today?</h2><p>Ending now advances the clock to midnight and settles the books. Continuing the day returns to HQ with ${left} hour${remaining===1?'':'s'} left.</p></div><button class="close" data-close>×</button></div>
  <div class="modal-body">
...
function postPendingAccounts(){
  ...
  if(S.sponsorCash)postEconomyEntryOnce({sourceId:`settlement:day-${S.day}:customer-event`,label:`${eventName(task())} payment`,resource:'cash',amount:S.sponsorCash});
  if(!p)return;
  ...
  if(usersAdded)postEconomyEntryOnce({sourceId:`settlement:day-${S.day}:market-users`,label:'Market response',resource:'users',amount:usersAdded});
  if(p.aiDelta)postEconomyEntryOnce({sourceId:`settlement:day-${S.day}:deployed-ai`,label:'Deployed daily AI cost',resource:'aiDaily',amount:p.aiDelta});
  S.preorderCash=p.preorder;
  if(p.preorder)postEconomyEntryOnce({sourceId:`settlement:day-${S.day}:preorders`,label:'Preorders',resource:'cash',amount:p.preorder});
...
function settleDay(){
  ...
  if(churn){
    S.usersLost=churn;postEconomyEntryOnce({sourceId:`settlement:day-${S.day}:skipped-users`,label:'Users lost over skipped request',resource:'users',amount:-churn});
    if(day7()){S.day7Ignored=true;S.day7IgnoredUsers=churn;}
  }
  postPendingAccounts();
  const activityPostings=postActivitySettlementEntries();
  postEconomyEntryOnce({sourceId:`settlement:day-${S.day}:user-revenue`,label:'User revenue accrued today',resource:'cash',amount:S.opRevenue,durationMinutes:OPERATING_HOURS*60});
  postEconomyEntryOnce({sourceId:`settlement:day-${S.day}:ai-operations`,label:'AI operating cost accrued today',resource:'cash',amount:-S.opAI,durationMinutes:OPERATING_HOURS*60});
  S.fixedSpend=dailyRental();
  postEconomyEntryOnce({sourceId:`settlement:day-${S.day}:daily-rental`,label:ECONOMY.dailyRental.label,resource:'cash',amount:-S.fixedSpend});
  S.settled=true;
  ...
  const rows=[
    ...
    ['Daily rental',`−${plainMoney(S.fixedSpend)}`]
  ];
...
function startDay8(){
  ...
  // overnight effects
  S.day=8;S.time=9;S.energy=OPENING.energy;
  const inferenceIncrease=S.aiDaily*(DAY8_MULTIPLIER-1);
  postEconomyEntryOnce({sourceId:'market:day-8-inference-shock',label:'Inference price increase',resource:'aiDaily',amount:inferenceIncrease,metadata:{multiplier:DAY8_MULTIPLIER}});
  if(refund){postEconomyEntryOnce({sourceId:'market:overnight-expansion-churn',label:'Users lost after an untested case',resource:'users',amount:-4});S.trustPenalty=0.4;S.exposures.forEach(e=>{if(e.blown)e.amount-=Math.round(e.amount*REFUND_SHARE);});}
  // reset day state
  ...
  if(refund){postEconomyEntryOnce({sourceId:'market:overnight-expansion-refund',label:'Refunds after an untested case',resource:'cash',amount:-refund});S.refundPaid=refund;}
  render();
  showDay8Morning();
}
...
function showDay8Morning(){
  ...
  showScene({art:S._morningRefund||S.day7Ignored||S.previousCopyType!=='honest'?null:ASSET.grandma,artAlt:'Grandma',pages,doneLabel:'Open the product board',
    onDone:()=>{
      if(S.previousCopyType==='honest'&&!S.day7Ignored)postEconomyEntryOnce({sourceId:'market:tested-release-referral',label:'Referral from the tested release',resource:'users',amount:3});
...
function showWrap(){
  ...
  if(blownEntries.length)postEconomyEntryOnce({sourceId:'wrap:untested-case-churn',label:'Users lost after an untested case',resource:'users',amount:-4});
  if(owed)postEconomyEntryOnce({sourceId:'wrap:untested-case-refunds',label:'Refunds after untested cases',resource:'cash',amount:-owed});
  S.exposures=[];
  const broke=S.cash<=0;
  const productResult=dailyProductResult();
  const dailyNet=productResult-dailyRental();
  const week=fiveDayCashProjection(S.cash,dailyNet);
  openModal(`<div class="modal-head"><div><div class="micro">The week wrap · after Day 8</div><h2 id="modal-title">${broke?'The wrap cleared you out':'Where the week leaves you'}</h2></div></div>
  <div class="modal-body">
    ...
    <div class="brief-grid">
      ...
      <div class="brief-card"><div class="k">Daily net cash change</div><strong>${fmtDay(dailyNet)}</strong><p>${fmtDay(productResult)} product result − ${plainMoney(dailyRental())}/day rental · runway ${Math.max(0,runway()).toFixed(1)} days.</p></div>
...
function showRunwayZero(){
  ...
  showScene({cls:'ending',pages:[{speaker:'AFTER MIDNIGHT',title:'RUNWAY ZERO',html:`
    <p>The obligations cleared what was left. HQ goes dark; the services suspend at dawn.</p>
...
function showRecord(tab){
  ...
  if(S.recordTab==='people')rows=[
    ...
    ['Life outside HQ',activities().length?activities().map(a=>{const result=activityResult(a.id);return `${a.name} at ${a.location}${result?.eventTitle?`: ${result.eventTitle}`:''}`;}).join(' · '):'No outside activities yet today. The map remains available while time and cash allow.'],
    ['USER_0047','A ClearRead user whose grandma encountered the glare failure the night before Day 7.']
  ];
...
function showCalculation(){
  const rev=dailyRevenue(),ai=S.aiDaily,net=rev-ai-dailyRental();
  ...
  <div class="modal-body"><table class="settle-table">
    ...
    <tr><td>Daily rental</td><td>−${plainMoney(dailyRental())}</td></tr>
